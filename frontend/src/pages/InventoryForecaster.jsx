...
function InventoryForecaster() {
  ...
  const fetchColumns = async (uploadedFile) => {
    ...
    try {
      const resp = await axios.post('http://localhost:8000/api/columns', formData);
      setAvailableColumns(resp.data.columns);
      if (resp.data.columns.length > 0) {
        setSelectedColumn(resp.data.columns[0]);
      }
    } catch (err) {
  ...
  return (
    <div className="app-container">
      <header>
        <h1>Inventory Forecaster</h1>
        <p className="subtitle">Forecast specific product inventory using Singular Spectrum Analysis</p>
      </header>
          ...
          <div className="controls">
            {availableColumns.length > 0 && (
              <div className="input-group">
                <label><FileText size={14} style={{display:'inline', marginRight:'4px', verticalAlign:'middle'}}/> Product</label>
                <select 
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis 
                    stroke="#94a3b8" 
                    tick={{ fill: '#94a3b8' }} 
                    label={{ value: selectedColumn || 'Value', angle: -90, position: 'insideLeft', offset: -15, fill: '#94a3b8', fontSize: 12 }}
                  />
                  <Tooltip 
